bw_to_vectorlist <- function(bwfile, genes_plus, genes_minus) {
...
k <- as.vector(by(genes_minus$V3, INDICES = genes_minus$V1, max))
n <- as.vector(by(genes_plus$V3, INDICES = genes_plus$V1, max))
q <- rbind(l,n,k)
l <- apply(q, 2, max)

read_dens <- sum(b$score)/sum(q[1,])
...
if (max(b$width)==1) {
  ...
  for (i in 1:length(l)) {
    ...
    print("For now this doesn't work yet with this bw file, because it neither has continues values nor only the values with width 1. I'm sure you can solve it if you look in the second approach (after else if) and combine it with the first approach")
  }
 

t <- list(t=t, read_dens=read_dens)
